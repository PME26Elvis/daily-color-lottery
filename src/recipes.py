...
def profile_bucket(row: dict[str, Any]) -> str:
    return str(row.get("source_profile_bucket") or (row.get("source_profile") or {}).get("profile_bucket") or "unknown")
...
def canonical_params(params: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for key, value in sorted((params or {}).items()):
        if isinstance(value, float):
            ...
            out[key] = float(value) if key not in {"shadow_tone", "highlight_tone"} else value
        else:
            out[key] = value
    return out


def stable_recipe_id(params: dict[str, Any] | None, style: str | None, algorithm: str | None, source_profile_bucket: str | None) -> str:
    payload = {
...
def recipe_from_output(row: dict[str, Any], created_at: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "source_profile": row.get("source_profile") or {"profile_bucket": bucket, "profile_tags": row.get("source_profile_tags", [])},
...
def promote_recipes_from_run(summary: dict[str, Any], existing: list[dict[str, Any]] | None = None, created_at: str | None = None) -> list[dict[str, Any]]:
    ...
    recipes = {r.get("recipe_id"): r for r in (existing or []) if isinstance(r, dict) and r.get("recipe_id")}
    for row in picks:
        recipe = recipe_from_output(row, created_at=created_at or summary.get("created_at"))
        recipes.setdefault(recipe["recipe_id"], recipe)
    return sorted(recipes.values(), key=lambda r: (float((r.get("score_summary") or {}).get("score") or 0), r.get("created_date") or ""), reverse=True)


def write_recipe_catalog(logs_dir: Path, docs_data_dir: Path, summary: dict[str, Any]) -> list[dict[str, Any]]:
...
def load_recipe(recipe_id: str, recipe_file: Path) -> dict[str, Any]:
    ...
    recipes = payload if isinstance(payload, list) else payload.get("recipes", []) if isinstance(payload, dict) else []
...
def build_recipe_analytics(runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for rid, items in by_recipe.items():
        ...
        rows.append({
            ...
            "average_score_when_reused": round(sum(scores) / len(scores), 2) if scores else None,
